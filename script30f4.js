    ...
    var MTV = (function(endpoint){
        ...
        return {
               ...
               if(self.data.curr !== "USD" && self.data.curr !== "EUR" && self.data.curr !== "CNY" && self.data.curr !== "GBP" && self.data.curr !== "CAD" && self.data.curr !== "RUB" && self.data.curr !== "HDK" && self.data.curr !== "JPY" && self.data.curr !== "AUD" && self.data.curr !== "CHF" && self.data.curr !== "PLN"      ){
                ...
                $.getJSON( "data.json", function(result){
                    if(typeof result["totalsupply"] != "undefined"){
                        ...
                        // };
						self.data["muedaily"] = result["muedaily"];
						self.data["usddaily"] = result["usddaily"];
						self.data["usdprice"] = result["usdprice"];
						// self.data["btcprice"] = result["btcprice"];
						self.data["mncount"] = result["mncount"];
						// self.data["totalsupply"] = result["totalsupply"];
						// self.data["pricebtcusd"] = result["pricebtcusd"];
                    };
                })
                .done(function() { self.connState('now', UP); })
                .fail(function(jqXHR, textStatus, errorThrown) { self.connState('now', DOWN); })
                .always(function() { self.data.polling -= 1; self.redraw(); });
				 ...
				 self.data.polling += 1;
				$.getJSON( "https://api.coinmarketcap.com/v1/ticker/pirl/", function(result){
					if(result){
						if(result.length>0){
							var coinmarketcapData=result[0];
							// for ( key in coinmarketcapData ) {
								// self.data[key] = coinmarketcapData[key];
							// };
							self.data["totalsupply"]  = coinmarketcapData.total_supply;
							self.data["btcprice"]     = coinmarketcapData.price_btc;
							self.data["pricebtcusd"] = coinmarketcapData.price_usd / coinmarketcapData.price_btc
            ...
            redraw: function(){
                ...
                switch(this.toggle) {
                    case true:
                        document.title = "PirlTV - " + fmtCurrShort(data.mue * data.fiatprice) + " " + this.data.curr;
                        break;
                    case false:
                        document.title = "PirlTV - " + fmtCurrShort(data.fiatprice) + " " + this.data.curr;
                        break;
                };
            ...
            updateData: function(o) {
                ...
                fmt.interest    = fmtCurr(data.muedaily * 365/MN_COIN_REQUIRED * 100, 2) + "%";
